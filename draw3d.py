...
def draw_silhouette(image, mask=None, poly=None):
    ...
    if mask is not None:
        ...
        img_mask = cv2.addWeighted(img_mask.astype(float), 1, mask.astype(float), 0.5, 0)
    if poly is not None:
        cv2.polylines(np.array(img_mask, dtype = np.int32), np.array([poly], dtype = np.int32), isClosed=True, thickness=2, color=(0, 0, 255))
...
def draw_2d_skeleton(image, pose_uv):
    ...
    #skeleton_overlay = (np.ones((224, 224, 3)) * 255).astype('uint8')
    marker_sz = 6
    line_wd = 3
    root_ind = 0
...
def save_a_image_with_mesh_joints(rgb_image, mask_image, mask_pred, poly, cam_param, mesh_xyz, face, pose_uv, pose_xyz, file_name, padding=0, ret=False):
    """
    :param mesh_plot:
    :param rgb_image: H x W x 3 (np.array)
    ...
    mask_pred_input = mask_pred.reshape(224, 224)
    
    
    img_mask = draw_silhouette(mask_image, mask_pred_input, poly)
    skeleton_overlay = draw_2d_skeleton(rgb_image, pose_uv)
    #rend_img_overlay = draw_mesh(rgb_image, cam_param, mesh_xyz, face)
    #rend_img_overlay2 = draw_mesh(mask_pred_input, cam_param2, mesh_xyz, face)
    skeleton_3d = draw_3d_skeleton(pose_xyz, mask_image.shape[:2])
    #mesh_3d = draw_3d_mesh(mesh_anno, mask_image.shape[:2], face)
    predict_mesh_3d = draw_3d_mesh(mesh_xyz, mask_image.shape[:2], face)

    #img_list = [rgb_image, img_mask, cv2.cvtColor(mask_pred, cv2.COLOR_GRAY2BGR), rend_img_overlay, rend_img_overlay2, mesh_3d, predict_mesh_3d, skeleton_3d]
    img_list = [img_mask, cv2.cvtColor(mask_pred, cv2.COLOR_GRAY2BGR), skeleton_overlay, predict_mesh_3d, skeleton_3d]

    image_height = mask_image.shape[0]
    image_width = mask_image.shape[1]
